...
blast_directory = f"{directory}/iofiles-blast"
pdb_directory = f"{directory}/iofiles-pdb"

#make output directories, presently unused; ought to be done in bash script since this is run multiple times
def makedirectories():
    #os.system(f"cd {directory}/iofiles-pdb")
    directories = ["moad-xml", "rcsb-pdb-holo", "rcsb-pdb-apo", "monomer-apo", "monomer-holo", "pairing-output", "filtering-log"] #, "qc_structures"
    for i in directories:
        os.system(f"mkdir {pdb_directory}/{i}")

def log_prot_rejection(reject_reason):
    with open(f"{pdb_directory}/filtering-log/protein-rejection-reasons.log", 'a') as f:
...
def extract_chain(struct, intype, outtype, chains):
    ...
    for line in open(f'{pdb_directory}/rcsb-pdb-{intype}/{struct}.pdb'): #read each line in file
    ...
    #save file
    with open(f"{pdb_directory}/monomer-{outtype}/{struct}-chain{'_'.join(chains)}.pdb", 'w') as f:
        f.writelines(buffer)
...
def getstruct(struct, extant, type):
    ...
    if struct not in extant:
        try: #download and load .pdb file
            os.system(f"wget https://files.rcsb.org/download/{struct}.pdb -O {pdb_directory}/rcsb-pdb-{type}/{struct}.pdb")
            ...
            os.system(f"rm {pdb_directory}/rcsb-pdb-{type}/{struct}.pdb")
    ...
    for line in open(f'{pdb_directory}/rcsb-pdb-{type}/{struct}.pdb'): #get pdb apo indices
...
def saveprot(xtal, id, chain, btype):
    ...
    try:
        xtal.atom_slice(xtal.top.select("protein")).save_pdb(f"paired-pdb-structures/{id}-chain{chain}-{btype}.pdb")
    ...
    for i in holo_ligand_list: #run through holo candidate ligands
        ...
        if holo_ligand_iis == {}: #calculate holo ligand/pocket indices if none were provided
            ...
            #    log_prot_rejection(f"{idh}_{chh}_{i[0]} could not be loaded by mdtraj")
                ...
                #with open(f"{directory}/filtering-log/protein_rejection_reasons.log", 'a') as f:
...
def getligs(struct, chain):
    ...
    if struct not in existing_xids_moad:
        os.system(f"wget https://www.bindingmoad.org/files/csv/{struct}.csv -O {pdb_directory}/moad-xml/{struct}.csv --no-check-certificate")
    ...
    with open(f'{pdb_directory}/moad-xml/{struct}.csv') as csvfile:
...
def get_indices(apo_id, apo_chain, holo_id, holo_chain):
    ...
    for line in open(f'{pdb_directory}/monomer-apo/{apo_id}-chain{apo_chain}.pdb'): #get pdb apo indices
    ...
    for line in open(f'{pdb_directory}/monomer-holo/{holo_id}-chain{holo_chain}.pdb'): #get pdb holo indices
        if line[0:5] == "ATOM " and line[13:16] == "CA ":
...
def check_gaps(type, pdb_id, chain, max_gap_len):
    ...
    for line in open(f'{pdb_directory}/monomer-{type}/{pdb_id}-chain{chain}.pdb'): #find first residue
    ...
    for line in open(f'{pdb_directory}/monomer-{type}/{pdb_id}-chain{chain}.pdb'): #track gap length
        pdblist = line.split()
...
blast_out = os.listdir(f"{blast_directory}/blast-output/") #filenames for blastp results of candidate holo (MOAD) structures with at least one candidate apo (PDB blastp hit) structure not itself from MOAD
...
def compare_chains(holo_chain_list, apo_chain_list, holo_sid, apo_sid, ha_type, max_gap_len):
    for a_chain in apo_chain_list: #for all the apo candidate bioassemblies
        print(f"{ha_type}apo_chain {a_chain}")
        if check_gaps("apo", apo_sid, a_chain, max_gap_len):
            apo = md.load(f"{pdb_directory}/monomer-apo/{apo_sid}-chain{a_chain}.pdb")
            ...
            for h_chain in holo_chain_list: #for all the holo candidate bioassemblies with ligands and no gaps
                ...
                holo = md.load(f"{pdb_directory}/monomer-holo/{holo_sid}-chain{h_chain}.pdb")
...
#the following were removed from the blast-output folder
...
existing_xids_moad = [i[0:4] for i in os.listdir(f"{pdb_directory}/moad-xml/")] #get a list of already-downloaded ligand lists to avoid downloading extra copies
existing_hids = [i[0:4] for i in os.listdir(f"{pdb_directory}/rcsb-pdb-holo/")] #get a list of already-downloaded candidate holo structures to avoid downloading extra copies
existing_aids = [i[0:4] for i in os.listdir(f"{pdb_directory}/rcsb-pdb-apo/")] #get a list of already-downloaded candidate apo structures to avoid downloading extra copies
...
for holo_candidate in range(rstart,rend): #loop through candidate holo structures
    ...
    apo_ids = [id.lower() for id in np.load(f"{blast_directory}/blast-output/{holo_id.upper()}-hits.npy")] #load blast results
    ...
    if isinstance(holo_chains_unfiltered, list): #avoid crashing when a structure doesn't load
        ...
        for h_chain in holo_chains_unfiltered[0]: #index of 0 is for true holo chains
            ...
            if check_gaps("holo", holo_id, h_chain, max_gap_len):
...
#-----------------------------------------------------
                ...
                if isinstance(apo_chains, list):
                    ...
                    compare_chains(holo_chains, apo_chains[0], holo_id, apo_id, "", max_gap_len)

            compare_chains(holo_chains, holo_chains_unfiltered[1], holo_id, holo_id, "h", max_gap_len)
            ...
            if buffer_pair_ligands != []:
                ...
                np.save(f"{pdb_directory}/pairing-output/ligand-pairs-{holo_candidate}", np.array(buffer_pair_ligands, dtype = object)) #save the ranked list of apo-holo pairs
...
np.save(f"{pdb_directory}/pairing-output/all-ligand-pairs-{rstart}-{rend}", np.array(pair_ligands, dtype = object))
lowrmsd_pairs = sorted(lowrmsd_pairs, key = itemgetter(4), reverse = True)
np.save(f"{pdb_directory}/pairing-output/all-lowrmsd-ligand-pairs-{rstart}-{rend}", np.array(lowrmsd_pairs, dtype = object))
