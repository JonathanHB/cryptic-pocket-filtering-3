...
for i in files:
    if i[6:11] == "index": #avoid extraneous files
        file = np.load(f"{npy_input_dir}/{i}", allow_pickle = True)
        print(file)
...
np.save(f"{output_dir}/screened-pairs-v{serial}", pairs)
