...
#get a list of previously filtered pockets
...
#holo_list = os.listdir(f"{directory}/{iofiles}/monomer-holo")
#holo_list2 = os.listdir(f"{directory}/{iofiles2}/monomer_holo")
...
for testind in range(len(all_pockets)):
    ...
    holo_xtal = md.load(f"{directory}/{iofiles}/monomer-holo/{all_pockets[testind][0]}-chain{all_pockets[testind][1]}.pdb")
    apo_xtal = md.load(f"{directory}/{iofiles}/monomer-apo/{all_pockets[testind][2]}-chain{all_pockets[testind][3]}.pdb")
    ...
    if lining_result == 1:
        ...
        abnormal_pockets.append(np.append(all_pockets[testind], sele_iis[3]))
        x_abnormal += 1
        #i_odd.append(testind)
    else:
...
print(f"{x_abnormal} abnormal pockets")
